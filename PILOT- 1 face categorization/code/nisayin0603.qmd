...
First option to pbtain the MAD MEDIAN ROLE for detecting outlires [link](https://rdrr.io/cran/Routliers/man/outliers_mad.html).

-   b is a scaling factor for the MAD to make it consistent for the normal distribution (the default is 1.4826
-   threshold is the cutoff value above which values are considered outliers (default is 3)
...
test <- data_participants |>
  ...
  mutate(n_trials = n()) |>
  mutate(n_bad_trials = 85*2 - n_trials) |>
...
Examining order effect first using regression analysis:
...
ggsave("plot1.png", plot = plot1, path = "../Plots/", width = 4000, height = 4000, units = "px")
...
## Demographics________________________________________________________________________________________________________
...
demo_wide_clean <- data_demo |>
  ...
  mutate(Participant_Private_ID = factor(Participant_Private_ID),
         ...
         data_demo$`vision-quantised` = factor(vision-quantised),
         data_demo$`language -quantised` = factor(language -quantised))

demo_wide_clean <- data_demo |>
  mutate(gender = case_when(
    `gender-quantised` == "1" ~ "man",
    `gender-quantised` == "2" ~ "woman"
  )) |>
  select(-`gender-quantised`, -`gender-text`, -`ethnic-text`, -`religiosity-quantised`, -`scale_of_SES-quantised`, -`age-quantised`) |>
  mutate(
    Participant_Private_ID = factor(Participant_Private_ID),
    age = as.numeric(age),
    children = as.numeric(children),
    scale_of_SES = as.numeric(scale_of_SES),
    `vision-quantised` = factor(`vision-quantised`),
    `language -quantised` = factor(`language -quantised`)
