...
density_plot2B <- ggplot(data_participants_without_all, aes(x = Response, fill = display)) + 
  geom_density(alpha = 0.5) + # Plot density
  geom_rug(aes(color = display), sides = "b") + # Add rug plot at the bottom
  scale_fill_brewer(palette = "Pastel1") + # Use Pastel1 palette for fill
  scale_color_brewer(palette = "Pastel1") + # Use Pastel1 palette for rug and mean line colors
...
#same but with outliers
order_effect_plotB <- ggplot(data_participants_without_all, aes(x = order_of_conditions, y = Response, fill = display)) +
  geom_boxplot() +
  stat_summary(fun = mean, geom = "errorbar", aes(ymax = ..y.., ymin = ..y..), width = 0.75, color = "red") +
  facet_wrap(~display, scales = "free") +
  labs(title = "Order Effect on Ratings",
...
mean_ratings_by_orderB <- data_participants_without_all |>
  group_by(order_of_conditions, display) |>
  summarise(mean_rating = mean(Response, na.rm = TRUE), .groups = 'drop') |>
  pivot_wider(names_from = display, values_from = mean_rating)


t_test_result_jewishB <- t.test(Response ~ order_of_conditions,
                               data = dplyr::filter(data_participants_without_all, display == "task_Jewish"),
                               alternative = "two.sided")

t_test_result_arabB <- t.test(Response ~ order_of_conditions,
...
t_test_results_without_outliresB <- data.frame(
  Display = c("Jewish", "Arab"),
  Statistic = c(t_test_result_jewishB$statistic, t_test_result_arabB$statistic),
  P_Value = c(t_test_result_jewishB$p.value, t_test_result_arabB$p.value)  # Difference of means, NA for the second row
)

# Create a table from the results
kable(t_test_results_without_outliresB, caption = "T-Test Results for Jewish and Arab Displays", format = "markdown")
...
data_imagesB <- data_participants_without_all |>
  group_by(image, Participant_Private_ID) |>
  #dplyr::filter(!(Participant_Private_ID %in% exclude_participant_IDs))|>
  dplyr::summarize(
    task_Jewish = mean(Response[display == "task_Jewish"], na.rm = TRUE),
...
data_images_big_diffB <- data_imagesB |>
  dplyr::filter(abs(avg_diff) >= 10) |>
  mutate(rated_ethnicity = case_when(avg_diff < 0 ~ "Arab",
                                     avg_diff > 0 ~ "Jewish",
                                     .default = NA))
data_images_choosenB <- data_imagesB |>
  dplyr::filter(abs(avg_diff)<10)|>
  mutate(rated_ethnicity = case_when(avg_diff < 0 ~ "Arab",
                                     avg_diff > 0 ~ "Jewish",
                                     .default = NA))
...
plot_imagesB <- data_participants_without_all |>
  group_by(display, image) |>
  mutate(per_condition_mean = mean(Response, na.rm = T)) |>
  #filter(str_detect(image, pattern = "20")) |>
  ggplot(aes(x = display, y = Response)) +
...
ggsave("plot_images_without_all.png", plot = plot_imagesB, path = "../Plots/", width = 4000, height = 4000, units = "px")
...
ggsave("plot1_without_all.png", plot = plot1B, path = "../Plots/", width = 5000, height = 5000, units = "px")
...
plot_participantsB <- data_participants_without_all |>
  group_by(Participant_Private_ID, display) |>
  mutate(participant_mean = mean(Response, na.rm = T)) |>
  ggplot(aes(x = display, y = Response)) +
  geom_point() +
...
ggsave("plot_participants_without_all.png", plot = plot_participantsB, path = "../Plots/", width = 4000, height = 4000, units = "px")
```
...
plot_participants_with_lineB <- data_participants_without_all |>
  ggplot(aes(x = display, y = Response)) +
  geom_smooth(aes(x = display, y = Response, group = image), method = "lm", color = "gray84", se = F, inherit.aes = F) +
  geom_smooth(aes(group = -1), method = "lm", se = F, color = "red") +
  facet_wrap(~Participant_Private_ID) +
...
ggsave("plot_line_participants_without_All.png", plot = plot_participants_with_lineB, path = "../Plots/", width = 4000, height = 4000, units = "px")
