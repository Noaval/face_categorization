...
data_participants <- rbind(data_categorization_roei,data_categorization_shani,
                           data_categorization_noa)|>
  colnames_to_underscores() |>
...
First option to pbtain the MAD MEDIAN ROLE for detecting outlires https://rdrr.io/cran/Routliers/man/outliers_mad.html

```{r}
#| code-fold: true

#code block 1
# Assuming outliers_mad is defined like this:
outliers_mad <- function(x, b = 1.4826, threshold = 3, na.rm = TRUE) {
  if(na.rm) {
    x <- na.omit(x)
  }
  mad_value <- mad(x, constant = b)
  median_value <- median(x)
  deviation <- abs(x - median_value) / mad_value
  return(deviation > threshold)
}
# Your dataset after preparing and filtering (e.g., data_participants)
data_participants <- data_participants |>
  mutate(is_outlier = ifelse(outliers_mad(Response, b = 1.4826, threshold = 3, na.rm = TRUE), 1, 0))

# To filter out outliers directly
data_cleaned <- data_participants %>% filter(is_outlier == 0)

# If you want to analyze a specific column and store results
response_outliers <- outliers_mad(data_participants$Response, b = 1.4826, threshold = 3, na.rm = TRUE)
```

-   Second option to pbtain the MAD MEDIAN ROLE for detecting outlires (better in my opinion)
...
Examining how the order of presenting conditions affects ratings of images as "Arab" or "Jewish," to ensure there is no influence of presentation sequence on perceptions.
...
order_effect <- lm(Response ~ order_of_conditions, data = data_participants)

print(parameters::model_parameters(order_effect))
```
...
ggplot(data_participants, aes(x = order_of_conditions, y = Response, fill = display)) +
  geom_boxplot() +
...
visualaizing image ratings in both display forms.
...
plot1 <- data_participants1 |>
  ...
  theme_classic() +
  scale_color_manual(values = c("same" = "#E84646", "no_same" = "#B26B8D", "mean" = "#7B8FD4"))


# Display the plot

ggsave("plotex.png", plot = plot1, path = "../Plots/", width = 4000, height = 4000, units = "px")
...
ggsave("plot_images.png", plot = plot_images, path = "../Plots/", width = 4000, height = 4000, units = "px")

```

![alt text: Overall trends per image by display](../Plots/plot_images.png)
...
plot_participants <- data_participants |>
...
ggsave("plot_participants.png", plot = plot_participants, path = "../Plots/", width = 1400, height = 1300, units = "px")
```

![alt text: Participants ratings](images/plot.png) Trying to add a line that will connect between the images
...
plot_participants1 <- data_participants |>
  ggplot(aes(x = display, y = Response)) +
...
male_per <- sum(demo_wide_clean$gender == "man")/nrow(demo_wide_clean)
female_per <- sum(demo_wide_clean$gender == "woman")/nrow(demo_wide_clean)
a_baniari_per <- sum(demo_wide_clean$gender == "לא בינארי")/nrow(demo_wide_clean)
