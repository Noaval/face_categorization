...
#| echo: false
#| eval: false
write_rds(model0.5_rt_invGauss, file = "../models/model0.5_rt_invGauss.rds")
```

```{r}
#| echo: false
model0.5_rt_invGauss <- read_rds("../models/model0.5_rt_invGauss.rds")
```
...
library(emmeans)
library(lme4)
(ems <- emmeans(model2.2_rt, ~ Ethnicity * identity, type = "response"))

ems_linear <- emmeans(model2.2_rt, ~ Ethnicity * identity)

ems_linear <- emmeans(model2.2_rt, ~ Ethnicity * identity)

ems_transformed <- transform(ems_linear, response = exp(estimate))
