...
model_variance <- as.data.frame(VarCorr(model0_rt_invGauss))

# Between-group variance (intercept variance for id)
between_variance <- model_variance$vcov[1]

# Within-group variance (residual variance)
residual_variance <- attr(model_variance, "sc")^2

# Calculate ICC
icc_value <- between_variance / (between_variance + residual_variance)
...
model_parameters(model1_rt_invGauss) |> insight::print_html()
...
model_parameters(model_maximal_rt)
```
...
```{r}
treat_data <- read.csv("C:/Users/97252/Documents/GitHub/face_cater/Study 1/data/data_exp_142922-v38_task-drsy.csv") |>
